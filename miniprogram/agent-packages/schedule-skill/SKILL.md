...
| 用户意图 | 必须调用 | 禁止动作 |
| --- | --- | --- |
| 看本周、下周、某周或某个日期所在周课表 | `getWeekSchedule` | 不要直接根据聊天上下文回答课表 |
...
1. 用户问某个具体日期的课表时，把该日期作为 `date` 传给 `getWeekSchedule`。
2. `getWeekSchedule` 的工具粒度是周视图；即使用户说“4月20号课表”，也应展示 4月20号所在周的 7 天课表。
3. 不要把日期查询解释成单日视图，不要只返回当天课程。
